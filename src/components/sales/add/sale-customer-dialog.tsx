...
import { CustomerForm } from '@/components/customers/add/customer-form'
...
export function SaleCustomerDialog({ open, onOpenChange, onCustomerCreated }: Props) {
  ...
  const handleSubmit = async (payload: NewCustomer) => {
    ...
    toast.push({
      ...
      description: `${created.name} foi salvo e selecionado na venda.`,
      type: 'success',
    })
    onOpenChange(false)
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Cadastre um cliente sem sair da venda em andamento.</DialogDescription>
        </DialogHeader>
